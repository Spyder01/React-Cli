    ...
    try {
        if (template === "default") {
            if (features.includes("Recoil") && features.includes("Router"))
                ...
                AppFile = `import React, {FC} from 'react';
                import {RecoilRoot} from 'recoil';
                import './App.css';
        ...
        const fileName = template === "typescript" ? `src/App.tsx`:(buildTools ==="webpack"?`src/App.js`:`src/App.jsx`);
        fs.writeFileSync(path.join(name, `src/App.${ext}`), AppFile);
